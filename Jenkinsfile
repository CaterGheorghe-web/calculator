                ...
                stage('Docker push')
                        {
                            steps
                            {
                                script
                                {
                                    ...
                                    sh("eval \$(aws ecr get-login-password --region eu-west-3 | docker login --username AWS --password-stdin 808995289075.dkr.ecr.eu-west-3.amazonaws.com/calculator | sed 's|https://||')")
